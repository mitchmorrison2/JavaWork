...
public class DealGame4 {
				...
				cases[caseHeld] = selectedCaseValue;
					...
					for (int x = 0; x < 24; x++) {
						...
						while (true) {
							...
							if (cases[casePicked] < 0) {
								selections[casePicked] = true;
								if (selections[casePicked] = true) {
						...
						input.nextLine();
								...
								//if they choose N, it just moves on
					} //end for
			} //end method
			...
			public static void displayRemainingValues() {
				 int[] remainingValues = Arrays.copyOfRange(cases, 0, cases.length);
				 Arrays.sort(remainingValues);
				 int selectedCaseIndex = Arrays.binarySearch(remainingValues, selectedCaseValue);
				 remainingValues[selectedCaseIndex] = selectedCaseValue * -1;
				 Arrays.sort(remainingValues);
				 int y = 0;
				 
				 for (int x = 0; x < remainingValues.length; x++) {
					if (remainingValues[x] > 0) {
						 System.out.printf("$%,9d  ", remainingValues[x]);
					y++;
					if (y == 5) {
						System.out.println();
					}
